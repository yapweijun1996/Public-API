...
describe('API catalog', () => {
  ...
  it('builds the twenty-nine third-expansion API requests', () => {
    ...
    expect(urls['ipwhois-lookup'].hostname).toBe('api.ip.sb')
    expect(urls['ipwhois-lookup'].pathname).toBe('/geoip/8.8.8.8')
    expect(urls['newton-math-solver'].pathname).toBe('/api/v2/simplify/2x%2B2x')
    expect(urls['gutendex-books'].hostname).toBe('openlibrary.org')
    expect(urls['gutendex-books'].pathname).toBe('/subjects/classics.json')
    expect(urls['gutendex-books'].searchParams.get('limit')).toBe('8')
    expect(urls['datamuse-rhymes'].searchParams.get('rel_rhy')).toBe('orange')
    expect(urls['open5e-monster-search'].hostname).toBe('www.dnd5eapi.co')
    expect(urls['open5e-monster-search'].pathname).toBe('/api/2014/monsters/young-red-dragon')
